...
from config import BASE_COMPOSITION, HARD_RULES, OPERABLE_UNIVERSE, SYSTEM_LIMITS
from engine import (
    classify_scenario, compute_score, compute_valuation, get_action_label,
    get_allocations, get_entry_label, get_macro_signal, get_new_money_plan,
    get_next_trigger, get_pause_mode, get_phase, get_rotation_plan,
    get_scenario_label, score_label,
)
...
def load_manual_macro():
    ...
    with open(MANUAL_MACRO_PATH, "r", encoding="utf-8") as f:
        raw = json.load(f)
    keys = ["cape", "cape_date", "pmi", "pmi_date", "lei_value", "lei_date", "lei_value_3m_ago", "lei_trend_3m", "per_global", "per_global_date", "per_global_source"]
    out = {}
    for k in keys:
        v = raw.get(k)
...
def compute_decision_block(data_freshness, nav_available, vix_available):
    ...
    if not vix_available:
        warnings.append("Sin VIX en tiempo real")
    thresholds = {
...
def main():
    ...
    action = get_action_label(scenario_code, drop_pct, vix, pause_mode)
    score, score_components = compute_score(cape, pmi, lei_trend_3m, vix)
    new_money_plan = get_new_money_plan(scenario_code)
    rotation_plan = get_rotation_plan(scenario_code, drop_pct, vix)
    ...
    decision_status = compute_decision_block(data_freshness, nav_available=nav is not None, vix_available=vix_available)
    ...
    payload = {
        ...
        "operational_checklist": ["¿Drawdown ≥ -10%?", "¿VIX > 20?", "¿Liquidez suficiente?", "¿No es dinero nuevo?"],
