...
async function loadDashboard() {
  ...
  function renderAll() {
    ...
    const newMoneyRule = auto.new_money_rule ? {
      rule: `${Math.round((auto.new_money_rule.invest_pct || 0) * 100)}% invertir / ${Math.round((auto.new_money_rule.reserve_pct || 0) * 100)}% reservar`,
      investPct: Number(auto.new_money_rule.invest_pct || 0),
      reservePct: Number(auto.new_money_rule.reserve_pct || 0),
      note: auto.new_money_rule.note || "Regla calculada en backend."
    } : getNewMoneyRuleData(scenarioData.newMoneyState);
