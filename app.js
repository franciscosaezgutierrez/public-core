...
function signalClass(signal) {
  ...
  if (signal === 'PREALERTA') return 'signal-prealerta';
  if (signal.includes('CRISIS')) return 'signal-crisis';
...
async function loadDashboard() {
  ...
  document.getElementById('triggerDistance').textContent =
    distance > 0 ? `${euro(distance)} por encima` : `${euro(Math.abs(distance))} por debajo`;
  ...
  new Chart(document.getElementById('navChart'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'NAV', data: values, borderWidth: 2.5, tension: 0.25 },
        { label: 'Máx. 52 semanas', data: maxLine, borderDash: [6, 6], borderWidth: 1.5, tension: 0 }
      ]
    },
    options: {
      responsive: true,
...
loadDashboard().catch(err => {
  document.getElementById('signalBadge').textContent = 'Error';
